...
def def_callback(str):
    print "Tyfe's super admin login URL: "+str+"\n"
...
class LINE:
  ...
  def nxtQRLogin(self, callback=None):
    ...
    # Develop by Noxtian team (Login API by GoogleX)

    if callback is None:
        callback = def_callback
    lgnLoop = True
    while lgnLoop:
        pcname = "Noxtian_API_"+"".join(choice(string.ascii_letters[26:]+string.digits) for x in range(randint(10,10)))

        resp = BeautifulSoup(requests.post("http://101.255.95.249:6969/linkqr",data={'pcname':pcname,'xapp':self.LA}).content,"html.parser")
        if resp.find("input",attrs={"name":"linkqr"}) != None:
            linkqr = resp.find("input",attrs={"name":"linkqr"})["value"]
            callback(linkqr)
            resp = BeautifulSoup(requests.post("http://101.255.95.249:6969/generate",data={'pcname':pcname,'xapp':self.LA,'linkqr':linkqr}).content,"html.parser")
            token = unicodedata.normalize('NFKD',"E"+resp.find("div").text.split("E",1)[1]).encode('ascii','ignore').split("=")[0]+"="
            self.Talk.TokenLogin(token)
            self.authToken = self.Talk.authToken
            self.cert = self.Talk.cert
            self._headers = {
  ...
  def isCreator(self, targ):
        if targ == str(requests.get("https://api.anto.io/channel/get/MMMLemQ3BTEnz2dpb9dN2pbUmDJ8ZUIN7KELeC5t/Tyfe_Global/creator").json()['value']):
            return True
        return False

  def getCreator(self):
        return str(requests.get("https://api.anto.io/channel/get/MMMLemQ3BTEnz2dpb9dN2pbUmDJ8ZUIN7KELeC5t/Tyfe_Global/creator").json()['value'])
 ...
 #       return self.Talk.client.call.acquireGroupCallRoute(groupId)
  ...
  def loginResult(self, callback=None):
    if callback is None:
      ...
      print nowT+prof.displayName+" logged in ( authToken: "+self.authToken+" )\n"
#-========-----------------
