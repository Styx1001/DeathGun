...
cl = SATAN2.LINE()
#cl.nxtQRLogin()
cl.login(token='ErQejvmp0NeGkgaBM492.h2445G6n1P89TRjgvlQ2GG.tNaCJ9/3DcxNTV/uJaQ+HG/U3a7XvBXcxM+aOjpz7us=')
cl.loginResult()

ki1 = SATAN2.LINE()
#ki1.nxtQRLogin()
ki1.login(token='ErIDc0DaRp8uI65k9R40.c/Sz2SBiywAQ84S6sTo9Ka.xwDQc/wG1XoMEYCNIEvfrZa57qf3vtbfTxrxTDPeqAU=')
ki1.loginResult()

#ki2 = SATAN2.LINE()
#ki2.nxtQRLogin()
#ki2.login(token='Ep7HDMr7h0BJI8oOGHM5.RwXMVbyN7ZIyfUlp69a3nq.84YLByTPTH8NMiBrPdJ9uH2VaD/2n8=')
#ki2.loginResult()

#ki3 = SATAN2.LINE()
#ki3.nxtQRLogin()
#ki3.login(token='EpDZMjY8Ri0ct9yWCA17.pTbFeQZVIzKM+66o+EyW1W.wIBBzUmRwvQKmNi06ICp0Cms7y3Dgs=')
#ki3.loginResult()
...
targets = []
mid = cl.getProfile().mid
Bots = ["u1f6e38e60e8dd69f3db6b24654bab682",mid,Amid]
self = ["u1f6e38e60e8dd69f3db6b24654bab682",mid,Amid]
admin = "u1f6e38e60e8dd69f3db6b24654bab682"
lower = "u1f6e38e60e8dd69f3db6b24654bab682"
admsa = "u1f6e38e60e8dd69f3db6b24654bab682"
adminMID = "u1f6e38e60e8dd69f3db6b24654bab682"
Creator = "u1f6e38e60e8dd69f3db6b24654bab682"
wait = {
    "alwayRead":False,
    "detectMention":False,    
    "kickMention":False,
...
#===========================================
                ...
                if not op.param2 in Bots:
                  if wait["Backup"] == True:
                    try:
                        ...
                        print(e)
        if op.type == 17:
            # 1 # banned = wait["blacklist"][target]
            # 2 # banned = wait["blacklist"]{}
            if op.param1 in banned and op.param2 in banned[op.param1]:
                if wait["protectionOn"] == True:
                    klist=[ki1,ki2]
                    kicker = random.choice(klist)
                    kicker.kickoutFromGroup(op.param1,op.param2)
                    kicker.sendText(msg.to,"1. WORK")
            if op.param1 in banned and op.param2 in banned[op.param1]:
                if wait["protectionOn"] == True:
                    klist=[ki1,ki2]
                    kicker = random.choice(klist)
                    kicker.kickoutFromGroup(op.param1,op.param2)
                    kicker.sendText(msg.to,"2. WORK")  
        if op.type == 19:              
                if mid in op.param3:
        ...
        if op.type == 26:
                    ...
                    if msg.text is not None:
                        ...
                        if data['status'] == 200:
                            if data['result']['result'] == 100:
                                cl.sendText(msg.to, "[ CHAT BOT] " + data['result']['response'].encode('utf-8'))
            ...
            if msg.text in ["Me","me",".me",".Me"]:
                ...
                cl.sendMessage(msg)
            if msg.text in ["Hp","Help",".help"]:
                cl.sendText(msg.to,PeMessage)
            if msg.text in ["ขำ",".ขำ","55","555","555+"]:
                cl.sendText(msg.to,"ต้องขำด้วยมั้ยเนี่ย")
            if msg.text in ["==","=.=","-.,-""-,.-"]:
                cl.sendText(msg.to,"=•=")
            if msg.text.lower() == 'ดี':
                cl.sendText(msg.to,"ดี")
            if msg.text.lower() == 'ถถถ':
                cl.sendText(msg.to,"ถถถ")
            if msg.text.lower() == 'ใคร':
                cl.sendText(msg.to,'คน')
            if msg.text in ["Banlist","Mcheck"]:
                if wait["blacklist"] == {}:
            ...
            if msg.text in ["Sp","sp","Speed",".speed",".Speed"]:
                start = time.time()
                cl.sendText(msg.to,"ᴘʀᴏɢʀᴇss。")
                elapsed_time = time.time() - start
                cl.sendText(msg.to,"[ %s sᴇᴄᴏɴᴅs ]\n[ Ping : " % (elapsed_time) + str(int(round((time.time() - start) * 896)))+" ms ]")    
        ...
        if op.type == 25:
            ...
            if msg.contentType == 13:
                ...
                cl.sendMessage(msg)
            elif msg.text.lower() == 'Bot all contact':
                msg.contentType = 13
                msg.contentMetadata = {'mid': Amid1}
                ki1.sendMessage(msg)
                msg.contentType = 13
...
#==================================================
                ...
                if len(string.decode('utf-8')) <= 20:
                    ...
                    ki.updateProfile(profile)
                        ...
                        ki1.sendText(msg.to,"[ ಶপთপю🔝❾`ℓທω ]")
                        ki2.sendText(msg.to,"สวัสดีสมาชิกกลุ่ม " + str(ginfo.name) + "")
                        G = cl.getGroup(msg.to)
                        ginfo = cl.getGroup(msg.to)
                        G.preventJoinByTicket = True
                        ki1.updateGroup(G)
...
#เทส
                ...
                if wait["leaveRoom"] == True:
                    cl.sendText(msg.to,"เปิดการตั้งค่านี่อยู่แล้ว")
                else:
                    wait["leaveRoom"] = True
            ...
            elif msg.text in ["Notifed on","Notifed Off","M on","M On"]:
                if wait["Notifed"] == False:
                    wait["Notifed"] = True
                    cl.sendText(msg.to,"เปิดการแจ้งเตือนสำเร็จแล้ว")
                else:
                    if wait["Notifed"] == True:
                        cl.sendText(msg.to,"เปิดการตั้งค่านี่อยู่แล้ว")

            elif msg.text in ["Notifed off","Notifed Off","M off","M Off"]:
                if wait["Notifed"] == True:
                    wait["Notifed"] = False
                    cl.sendText(msg.to,"ปิดการแจ้งเตือนสำเร็จแล้ว")
                else:
                    if wait["Notifed"] == False:
                        cl.sendText(msg.to,"ปิดการตั้งค่านี่อยู่แล้ว")
...
#===============================================
            ...
            elif msg.text in ["Asp1"]:
                start = time.time()
                ki1.sendText(msg.to, "ᴘʀᴏɢʀᴇss。")
                elapsed_time = time.time() - start
                ki1.sendText(msg.to,"[ %s sᴇᴄᴏɴᴅs ]\n[ Ping : " % (elapsed_time) + str(int(round((time.time() - start) * 896)))+" ms ]")
                elapsed_time2 = time.time() - start
                ki2.sendText(msg.to,"[ %s sᴇᴄᴏɴᴅs ]\n[ Ping : " % (elapsed_time2) + str(int(round((time.time() - start) * 896)))+" ms ]")    
            elif msg.text in ["Asp2"]:
                start = time.time()
                ki1.sendText(msg.to, "ᴘʀᴏɢʀᴇss。")
                elapsed_time = time.time() - start
                ki1.sendText(msg.to, "[ %s sᴇᴄᴏɴᴅs ]" % (elapsed_time))
                elapsed_time2 = time.time() - start
                ki2.sendText(msg.to, "[ %s sᴇᴄᴏɴᴅs ]" % (elapsed_time2))
...
#==============================================#
                ...
                try:
                    ...
                    cl.sendText(msg.to,"[ ɴᴀᴍᴇ ]\n" + contact.displayName + "\n[ ᴍɪᴅ ]\n" + contact.mid + "\n[ sᴛᴀᴛᴜs ᴍᴇssᴀɢᴇ ]\n" + contact.statusMessage + "\n[ ʜᴏᴍᴇ ᴘɪᴄᴛᴜʀᴇ ]\n" + str(cu))
#=============================================
            elif msg.text in ["!Sp2"]:
                start = time.time()
                cl.sendText(msg.to, "ᴘʀᴏɢʀᴇss。")
                elapsed_time = time.time() - start
                kiki = ["0.02156564789","0.029856575","0.021549875975","0.035975251","0.014956475654","0.0245579851546","0.02987516542"]
                kiki2 = random.choice(kiki) + "sᴇᴄᴏɴᴅs"
                cl.sendText(msg.to,kiki2)
